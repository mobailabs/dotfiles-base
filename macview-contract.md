...
**落地位置**:这一页是 macview 的**「系统」一级对象**(框架 §13,`prefs → System`)。
它的界面**没有查询、没有四态结论** —— 只有一段说明 + 一个「应用系统设置」按钮
(走 `install.zsh prefs`,要提权)。说明里那几个主题名是**举例**,不是从
`PREF_ORDER` 读的清单。
...
⚠️ 早先这张表里还有一行「prefs 主题数(9)」。**已删** —— 因为「系统」页
（框架 §13）**不报偏好状态**（契约 §2.7），所以它**不需要**那个数。那一页
只把几个主题名当**举例**写在说明里（`SystemPage.topicExamples`），
**明确标注「不是清单」** —— 不是从 `PREF_ORDER` 抄来的真相,所以漂移
**没有后果**（不像落点数、包清单,写错了会让人以为「齐了」）。
若将来做偏好状态,唯一不漂移的做法仍是让每个 `prefs.d/*.zsh` 自己报键名。
